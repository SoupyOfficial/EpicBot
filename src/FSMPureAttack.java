...
@ScriptManifest(name = "Pure Attack FSM 1.0", gameType = GameType.OS)
public class FSMPureAttack extends LoopScript {
    ...
    protected int loop() {
        ...
        NPC chicken = getAPIContext().npcs().query().nameMatches("Chicken").interactingWithMe().reachable().results().nearest();
        //search for chicken
        if(!myPlayer().isInCombat() && chicken == null) {
            chicken = getAPIContext().npcs().query().nameMatches("Chicken").actions("Attack").reachable().results().nearest();
            delay();
            //Attack chicken
            if (chicken != null && !chicken.isVisible()) {
                getAPIContext().camera().turnTo(chicken.getLocation().randomize(2, 2, 2));
            } else if (chicken != null && chicken.isVisible()  && chicken.hasAction("Attack")) {
                System.out.println("Attack Chicken");
                NPC finalChicken = chicken;
                delay(() -> finalChicken.interact("Attack"));
            }
        }
        return 600;
    }
    ...
    public void doAttackChicken() {
        ...
        //search for chicken
        if(!myPlayer().isInCombat() && chicken == null) {
            chicken = getAPIContext().npcs().query().nameMatches("Chicken").health(3).reachable().results().nearest();
    ...
    private boolean isPlayerInteracting() {
        return getAPIContext().localPlayer().isAnimating() || getAPIContext().localPlayer().isMoving() || getAPIContext().localPlayer().getInteracting() != null || getAPIContext().localPlayer().getAnimation() != -1;
    }

    public void delay() {
        Time.sleep(5000, this::isPlayerInteracting);
        Time.sleep(Random.nextInt(600, 1800));
    }
